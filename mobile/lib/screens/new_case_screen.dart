...
class NewCaseScreen extends StatefulWidget {
  ...
  const NewCaseScreen({super.key, required this.services, required this.mediator});
...
class _NewCaseScreenState extends State<NewCaseScreen> {
  ...
  void initState() {
    super.initState();
    _locationController.text = '${widget.mediator.locality}, ${widget.mediator.region}';
  ...
  Future<void> _toggleListening() async {
    ...
    if (!ready) {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Microphone permission is needed for voice intake.')));
      return;
    }
    ...
    await _speechService.startListening(
      ...
      onResult: (text, isFinal) {
        final combined = _voiceBaseText.isEmpty ? text : '$_voiceBaseText $text';
  ...
  Future<void> _toggleVoiceNoteRecording() async {
    ...
    if (!ready) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Microphone permission is needed to record a voice note.')));
      return;
    }
  ...
  Future<void> _save() async {
    ...
    final referralCheck = await widget.services.referralService.check(description);

    await widget.services.db.into(widget.services.db.cases).insert(CasesCompanion.insert(
          id: const Uuid().v4(),
          caseType: _caseType,
          partiesJson: jsonEncode(_partyRoles.map((r) => {'role': r}).toList()),
          description: description,
          voiceNoteRefsJson: Value(_voiceNoteFilePaths.isEmpty ? null : jsonEncode(_voiceNoteFilePaths)),
    ...
    if (referralCheck.flagged) {
      await showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('${referralCheck.reason}\n\nSuggested next step: ${referralCheck.suggestedNextStep}'),
          actions: [
            TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Noted')),
          ],
        ),
      );
    }
          ...
          children: [
            ...
            Row(
              children: [
                ...
                IconButton.filled(onPressed: _addPartyRole, icon: const Icon(Icons.add)),
              ],
            ),
              ...
              Padding(
                ...
                child: Wrap(
                  ...
                  children: _partyRoles
                      .map((r) => Chip(
                            label: Text(r),
                            onDeleted: () => setState(() => _partyRoles.remove(r)),
                          ))
                      .toList(),
                ),
              ),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Location / area', border: OutlineInputBorder()),
              ...
              children: [
                Expanded(
                  child: TextFormField(
                    ...
                    validator: (v) => (v == null || v.trim().length < 5) ? 'Please describe the case' : null,
                  ),
                ),
              ],
            ),
            ...
            Row(
              children: [
                SegmentedButton<SpeechLanguage>(
                  ...
                  onSelectionChanged: (s) => setState(() => _speechLanguage = s.first),
                ),
                const Spacer(),
                FilledButton.icon(
            ...
            FilledButton.icon(
              ...
              icon: Icon(_recordingVoiceNote ? Icons.stop : Icons.fiber_manual_record),
              label: Text(_recordingVoiceNote
                  ? 'Stop (${_formatDuration(_recordingElapsed)})'
                  : (_voiceNoteFilePaths.isEmpty ? 'Record' : 'Record another')),
              style: _recordingVoiceNote ? FilledButton.styleFrom(backgroundColor: Colors.red) : null,
            ),
