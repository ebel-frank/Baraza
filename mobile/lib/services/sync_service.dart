...
class SyncService {
  ...
  Future<SyncResult> pushPending() async {
    ...
    if (session == null) {
      return SyncResult(success: false, pushedCount: 0, error: 'Not signed in.');
    }

    final online = await connectivityService.isOnline();
    if (!online) {
      return SyncResult(success: false, pushedCount: 0, error: 'Device is offline.');
    }
    ...
    final payload = {
      'cases': pending
          .map((c) => {
                ...
                'voiceNoteRefs': c.voiceNoteRefsJson == null ? [] : jsonDecode(c.voiceNoteRefsJson!),
                'location': c.location,
                'createdAt': c.createdAt.toIso8601String(),
                'referralFlag': c.referralFlag,
                'referralReason': c.referralReason,
                'advisoryResponse':
                    c.advisoryResponseJson == null ? null : jsonDecode(c.advisoryResponseJson!),
              })
          .toList(),
    };
    ...
    try {
      ...
      if (response.statusCode != 200 && response.statusCode != 201) {
        return SyncResult(
            success: false, pushedCount: 0, error: 'Backend returned ${response.statusCode}');
      }
  ...
  Future<SyncResult> pullFromServer() async {
    ...
    try {
      ...
      for (final c in serverCases) {
        await db.upsertFromServer(CasesCompanion(
          ...
          voiceNoteRefsJson: Value(
              (c['voiceNoteRefs'] as List?)?.isNotEmpty == true ? jsonEncode(c['voiceNoteRefs']) : null),
          location: Value(c['location'] as String),
          createdAt: Value(DateTime.parse(c['createdAt'] as String)),
          syncedAt: Value(c['syncedAt'] == null ? null : DateTime.parse(c['syncedAt'] as String)),
          referralFlag: Value(c['referralFlag'] as bool),
          referralReason: Value(c['referralReason'] as String?),
          advisoryResponseJson:
              Value(c['advisoryResponse'] == null ? null : jsonEncode(c['advisoryResponse'])),
        ));
      }
